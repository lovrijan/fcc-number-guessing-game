...
if [[ -z $USERNAME_AVAIL ]]
  then
    ...
    echo "Welcome back, $USERNAME! You have played $GAMES_PLAYED games, and your best game took $BEST_PLAY guesses."
fi
...
do
  if [[ ! $NUM =~ ^[0-9]+$ ]]
    then
      ...
      if [[ $NUM -eq $RANDOM_NUMBER ]] #if guess is equal to random number
      then
        break;
      else
        if [[ $NUM -gt $RANDOM_NUMBER ]] #if guess is greater than random num
        then
          echo -n "It's lower than that, guess again:"
        elif [[ $NUM -lt $RANDOM_NUMBER ]] #if guess is lower than random num
          then
